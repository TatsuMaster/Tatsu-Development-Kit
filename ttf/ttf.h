...
#define _display_ts_name(ts_name) printf("Test Step %d: \t%s...", t_step_counter, ts_name);
...
#define TEST_SUITE_INIT() begin = clock(); prepare_console_info(); t_step_counter = 0; t_step_pass_counter = 0; t_step_fail_counter = 0; printf("Tatsu Test Framework - TDK Version %d.%d.%d\n", TDK_MAJOR_VERSION, TDK_MINOR_VERSION, TDK_PATCH_LEVEL);
#define TEST_SUITE_END() end = clock(); printf("\n\nExecuted %d test steps in %f seconds\n\n", t_step_counter, (double)(end - begin) / CLOCKS_PER_SEC); printf("Tests passed: %d\nTests failed: %d\nTest errors : %d\n\n", t_step_pass_counter, t_step_fail_counter, 0);
